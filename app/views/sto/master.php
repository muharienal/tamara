...
<div class="container mt-5">
  ...
  <style>
    .uploader { border:2px dashed #cfe3ff; border-radius:12px; background:#f7fbff; cursor:pointer; }
    .uploader:hover { background:#f1f8ff; }
    .uploader .cloud { font-size:40px; line-height:1; }
    .uploader .cta { color:#1976d2; font-weight:600; }
    .file-pill { display:flex; align-items:center; gap:.5rem; padding:.45rem .6rem; border:1px solid #e5e7eb; border-radius:8px; }
    .file-badge { font-weight:700; font-size:.75rem; padding:.15rem .45rem; border-radius:6px; background:#e8f1ff; color:#0b5ed7; text-transform:uppercase; }
    .file-remove { border:none; background:#f8d7da; color:#a61b2b; width:26px; height:26px; border-radius:50%; font-weight:700; line-height:1; }
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  document.querySelectorAll('.filter-input').forEach(inp=>{
    inp.addEventListener('input',()=>{
      const cols = { 'f_nomor':1, 'f_gudang':3, 'f_transaksi':4, 'f_status':9 };
      ...
      Array.from(tb.rows).forEach(r=>{
        ...
        for(const id in cols){
          ...
          if (val && !r.cells[cols[id]].innerText.toLowerCase().includes(val)) { show = false; break; }
        }
  ...
  function initMultiUploader(zoneId, inputId, listId, options = {}) {
    ...
    function renderList() {
      ...
      Array.from(dt.files).forEach((f, idx) => {
        const li = document.createElement('li'); li.className = 'file-pill';
        li.innerHTML = `
    ...
    function acceptFiles(files) {
      Array.from(files).forEach(f => {
        ...
        if (!ALLOWED.includes(ext)) { console.warn('Tipe tidak diizinkan:', f.name); return; }
        if (f.size > MAX_BYTES)   { console.warn('Kebesaran:', f.name); return; }
        dt.items.add(f);
      });
  ...
  document.getElementById('editForm').addEventListener('submit',e=>{
    ...
    fetch('index.php?page=master_sto',{ method:'POST', body:data })
      .then(r=>r.json())
      .then(res=>{
